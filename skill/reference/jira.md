...
Classic workflow management — `/rest/api/3/workflow` (B837–B845 + B934 archived as already-covered), bulk workflow API — `/rest/api/3/workflows` (B846–B850), and transition property management (B935–B938, deprecated).

**Pagination:** `list` is offset-paginated (`--start-at`, `--max-results`). The usage actions (`issue-type-usages`, `project-usages`, `workflow-scheme-usages`) use cursor pagination (`--next-page-token`, `--max-results`). `get-rule-config` is offset-paginated (`--start-at`, `--max-results`, max 50 per page).

**Note:** `GET /rest/api/3/workflow/search` (B934) is implemented as `list` and was already present before this PR.

**Required body endpoints (`--body` JSON):** `bulk-get`, `bulk-create`, `validate-create`. For `bulk-create` and `validate-create`, the body is a deeply nested JSON object — pass it as a JSON string via `--body`.
