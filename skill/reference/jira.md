...
Field configuration scheme management (B914–B917). Covers the flat `/rest/api/3/fieldconfigurationscheme` surface: paginated listing, create, delete, and update. Marked deprecated by Atlassian but still operational on Jira Cloud. Distinct from `fieldconfiguration` (`/rest/api/3/fieldconfiguration`) and `config` (`/rest/api/3/config/fieldschemes`).

| Action   | Positional | Required flags | Optional flags                               |
| -------- | ---------- | -------------- | -------------------------------------------- |
| `list`   | —          | —              | `--start-at`, `--max-results`, `--ids` (CSV) |
| `create` | —          | `--name`       | `--description`                              |
| `update` | `<id>`     | `--name`       | `--description`                              |
| `delete` | `<id>`     | —              | —                                            |

```bash
# Paginate all field configuration schemes
...
# Update a scheme (API requires name)
