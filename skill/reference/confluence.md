...
| `create`                      | —          | `--space-id`, `--title`, `--body`                       | `--embedded`, `--private`, `--root-level`                                                                                                                                                                                                                                                                                                                    |
...
- `create --embedded` tags the page for NCS, `--private` restricts it to the creator, and `--root-level` creates it outside the space homepage tree. A root-level request cannot have a `parentId` in the SDK body.
...
- `list --sort` accepts `BlogPostSortOrder`: `id`, `-id`, `created-date`, `-created-date`, `modified-date`, `-modified-date`. `--status` is a comma-separated subset of `current`, `deleted`, `trashed` (the collection filter is narrower than the single-blog-post `get` lookup, which also accepts `draft`/`historical`). `--body-format` accepts `storage` or `atlas_doc_format`. `--title` filters by exact title.
