...
- `set-role-assignments --value` accepts a JSON array of `{ principal: { principalType, principalId }, roleId }` entries; the server replaces the space's role assignments wholesale with the provided list.
