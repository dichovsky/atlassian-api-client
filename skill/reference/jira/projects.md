...
| Action                        | Positional                         | Required flags                          | Optional flags                                                                                                                                                                                             |
| ----------------------------- | ---------------------------------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list`                        | —                                  | —                                       | `--start-at`, `--max-results`, `--order-by`, `--expand` (CSV), `--status` (CSV), `--type-key`, `--id` (CSV integers), `--keys` (CSV), `--query`, `--category-id`, `--property-query`, `--properties` (CSV) |
| `get`                         | `<projectKeyOrId>`                 | —                                       | —                                                                                                                                                                                                          |
...
- `list` calls the current `/project/search` endpoint. `--status`, `--id`, `--keys`, and `--properties` are comma-separated and are sent as repeated query parameters; `--expand` stays a comma-separated expansion string on the wire. The removed legacy `--action` filter has no current replacement and should be dropped.
...
atlas jira projects list --type-key software --status live --query platform --order-by key --max-results 50
