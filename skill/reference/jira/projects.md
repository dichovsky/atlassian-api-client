...
- `list` calls the current `/project/search` endpoint. `--status`, `--id`, `--keys`, and `--properties` are comma-separated and are sent as repeated query parameters; `--expand` stays a comma-separated expansion string on the wire. `--action` accepts `view` (default), `browse`, `edit`, or `create` and replaces the same filter removed from `list-legacy`. `--order-by` accepts `category`, `key`, `name`, `owner`, `issueCount`, `lastIssueUpdatedDate`, `archivedDate`, or `deletedDate`, optionally prefixed with `+` or `-`.
