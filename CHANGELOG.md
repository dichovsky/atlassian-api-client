...
- Removed Jira workflow-transition-property SDK methods `getTransitionProperties`, `createTransitionProperty`, `updateTransitionProperty`, and `deleteTransitionProperty`, plus the four matching CLI actions, because Atlassian removed `/rest/api/3/workflow/transitions/{transitionId}/properties` from the public API on 2026-07-13. There is no one-to-one replacement: read `transitions[].properties` through `workflows.bulkGet()` / `atlas jira workflows bulk-get --body …`, then submit a full versioned definition through `workflows.updateWorkflows()` / `atlas jira workflows update --body …` (optionally validate first with `validate-update`).
- Removed the Confluence `ContentPropertiesResource` and the reachable `client.contentProperties.*` methods, the package-root `ListContentPropertiesParams` / `UpdateContentPropertyData` exports, and the five matching `confluence.contentProperties.*` scope-map operations. Migrate page and blog-post property calls to `client.pages.listProperties` / `client.pages.*Property` or `client.blogPosts.listProperties` / `client.blogPosts.*Property`, using the shared `ListSharedContentPropertiesParams` / `UpdateSharedContentPropertyData` types.
- Corrected `IssueArchiveExportData` to the live `ArchivedIssuesFilterRequest` contract. The fictional `jql` and `exportType` fields are gone; use `archivedBy`, `archivedDateRange`, `issueTypes`, `projects`, and `reporters`; both fields of `DateRangeFilterRequest` are now required when a range is present. The removed CLI `--jql` and `--export-type` selectors now fail with migration guidance instead of silently widening the export; Atlassian always produces CSV. A successful CLI call returns the live `ExportArchivedIssuesResult` task envelope instead of the former synthetic `{ submitted: true }`; poll its task ID with `atlas jira task get <taskId>`.
- `atlas jira search search` (and the actionless/`query` aliases) now use the current cursor-paginated `/rest/api/3/search/jql` endpoint and forward `--next-page-token`. Replace offset-only automation with token pagination. The printed JSON is now the `JqlSearchResult` envelope: `total`, `startAt`, and `maxResults` are no longer present; use `atlas jira search approximate-count --jql …` when a count is required. Current cursor actions reject the legacy `--start-at` / `--validate-query` flags, and POST search actions reject GET-only `--fail-fast`, instead of silently dropping them; SDK GET-only options now live on `JqlSearchGetParams` / `LegacySearchGetParams` rather than their shared POST parameter types. The legacy routes remain explicitly reachable as `legacy-post` and `get`, both deprecated; undocumented arbitrary action names no longer fall through to a search and now error.
- Removed unsupported dashboard-list parameters from `ListDashboardsParams`; `atlas jira dashboards list --order-by` and `--expand` now fail with migration guidance. Use `dashboards.search` / `atlas jira dashboards search` for sorting and expansion.
- Removed unsupported legacy-project-list parameters from `ListLegacyProjectsParams`: `maxResults`, `orderBy`, `startAt`, `typeKey`, `categoryId`, `action`, and `query`. Its `expand` field is now one comma-separated string rather than `string[]`. The matching CLI flags now fail with guidance; use `projects.list()` / `atlas jira projects list` for the full current `/project/search` filter and pagination surface, including `action`.
- Removed unsupported `maxResults`, `orderBy`, `query`, and `status` fields from `ListAllProjectVersionsParams`; the matching `atlas jira projects list-all-versions` flags now fail with guidance. Use `projects.listVersions` / `atlas jira projects list-versions` for filtered or paginated results.
- `GroupUserPickerResource.pick()` now requires params with a non-optional `query`; the never-serialized `projectRole` and `excludeAccountIds` fields were removed from `GroupUserPickerParams`. In the CLI, project scoping requires `--field-id` together with `--project-id`; removed `--project-role` produces targeted migration guidance, and misplaced `--exclude-account-ids` points to `atlas jira users picker`. Invalid `--avatar-size` values now fail locally instead of silently falling back server-side.
- `boards.listAll()` now forwards `accountIdLocation`, `projectLocation`, `includePrivate`, `negateLocationFiltering`, `orderBy`, `expand`, `projectTypeLocation`, and `filterId`, which v3.0.0 accepted but silently dropped. Existing calls using those filters can now return a different, correctly filtered board set; re-check pagination consumers that supplied them.
- Tightened current response/request types: `Space.type` / `Space.status` and `ListSpacesParams.type` / `ListSpacesParams.status` now use the live Confluence enums, `Space.description`, `Space._links`, and inline expansion envelopes use their documented response shapes, `ListLabelsParams.prefix` now uses the documented `LabelPrefix` enum, and `GetPageParams.status` no longer accepts the unsupported `any` value. Jira `ListProjectsParams.orderBy` and `ListBoardsParams.orderBy` now use the documented token sets, custom-template `role` payloads now use the spec's capability-role object (or `null`), field-scheme search parameters are response-specific and optional, and `JqlSearchResult.warnings[].details` now has the documented limit-warning shape instead of `unknown`. Update call sites, fixtures, or mocks that relied on the former open-ended records or guaranteed response fields.
- The CLI now rejects the orphaned Confluence `--database-id` option (pass the database ID positionally) and rejects Jira-only `--software-cloud-id` on Confluence commands instead of silently ignoring either option.
...
- Full Jira search CLI/skill parameter parity across current cursor and deprecated offset actions, including properties, field-key mode, reconciliation IDs, archived-project inclusion, fail-fast, expansion, validation, and offset controls.
- Current Confluence page-create request shapes through the additive `CreatePageRequest` input union: draft creation without a title, flat or nested write bodies, parent/status/subtype fields, matching CLI/skill flags including JSON body input, and package-root exports for all new Page and Space contract types. The existing `CreatePageData` interface remains extension-compatible.
- Complete spec-derived scope validation: 247 unique granular scopes referenced by operations, 8 additional granular catalog entries, and 16 classic or Jira Software compatibility scopes (271 recognized scopes total). Operation recommendations remain granular.
- Canonical OpenAPI contract fingerprints and deterministic endpoint-gap regression checks. The weekly drift job now detects route, parameter, schema, response, security, and executable SDK route-coverage changes instead of only type-generator failures; invalid/empty operations, malformed or dangling response entries, root/path/operation server-scope drift, ambiguous or shadowed base aliases, transformed route expressions, unsafe helper returns or path reassignments, shadowed pagination/query imports, dead or non-public calls/wiring, nested/unwired/duplicate resource classes, conditional or unrecognized client-resource mutations, computed request keys, unresolved Path Item references, unexpected SDK routes, and request-like comments, literals, regexes, or escaped identifiers fail closed. All eight OpenAPI HTTP operation keys are covered, and the Confluence v1 exception is limited to the exact attachment-upload operation.
