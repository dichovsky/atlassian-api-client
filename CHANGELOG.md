...
- **errors (B026)** — `ResponseTooLargeError` exported from `src/core/index.ts`. Carries `limitBytes` (the configured cap that was exceeded) and an optional `status` (the upstream HTTP status when the overflow occurred on the error path, so callers can still classify the originating response as 2xx vs 4xx/5xx). Not an `HttpError` — it preempts the would-be `HttpError` on the error path rather than being one.
