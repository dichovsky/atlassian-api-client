...
- Canonical OpenAPI contract fingerprints and deterministic endpoint-gap regression checks. The weekly drift job now detects route, parameter, schema, response, security, and executable SDK route-coverage changes instead of only type-generator failures; comments and request-like quoted, template, or regex literals cannot satisfy coverage or shadow client wiring.
