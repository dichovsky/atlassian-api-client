...
- Current Confluence page-create request shapes: draft creation without a title, flat or nested write bodies, parent/status/subtype fields, and matching CLI/skill flags including JSON body input.
...
- Canonical OpenAPI contract fingerprints and deterministic endpoint-gap regression checks. The weekly drift job now detects route, parameter, schema, response, security, and executable SDK route-coverage changes instead of only type-generator failures; invalid/empty operations, root/path/operation server-scope drift, ambiguous or transformed route expressions, unsafe helper returns or path reassignments, dead calls/wiring, computed request keys, unresolved Path Item references, unexpected SDK routes, and request-like comments, literals, or regexes fail closed. All eight OpenAPI HTTP operation keys are covered, and the Confluence v1 exception is limited to the exact attachment-upload operation.
