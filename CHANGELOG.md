...
- **types/space** — `Space` adds `authorId`, `currentActiveAlias`, typed `icon` (`SpaceIcon`), typed `description` (`SpaceDescription` with `plain`/`view` representations). `ListSpacesParams` adds `ids[]`, `labels[]`, `favorited-by`, `not-favorited-by`, `sort` (`SpaceSortOrder`), `description-format`, `include-icon`. (B025)
- **types/blog-post** — `BlogPost` adds `ownerId`, `lastOwnerId`, `parentId`, typed `parentType`, `position`. `ListBlogPostsParams` adds `id[]`, `space-id[]`, `sort` (`BlogPostSortOrder`). (B026)
- **types/comment** — `FooterComment` adds `parentCommentId`, `attachmentId`, `customContentId`. `InlineComment` adds typed `resolutionStatus` (`InlineCommentResolutionStatus`), `resolutionLastModifierId`, `resolutionLastModifiedAt`, `parentCommentId`, typed `properties` (`InlineCommentProperties`). List params accept `sort` (`CommentSortOrder`). Create payloads accept `parentCommentId`. (B027)
- **types/attachment** — `Attachment` adds `fileId`, `customContentId`, `createdAt`. `ListAttachmentsParams` adds `sort` (`AttachmentSortOrder`), `status: string | readonly string[]`. (B028)
- **types/content-property** — `ContentProperty.value` (and create/update value) tighten from `unknown` to recursive `JsonValue` union. New `validateContentPropertyKey(key)` (`src/confluence/validators.ts`) throws `ValidationError` when the key is empty, exceeds 255 chars, or contains characters outside `^[A-Za-z0-9_.-]+$`. Validator runs inside `createForPage` / `updateForPage`. Validator + `CONTENT_PROPERTY_KEY_PATTERN` + `CONTENT_PROPERTY_KEY_MAX_LENGTH` exported. (B029)
- **types/custom-content** — `CustomContent` adds `customContentId` (parent reference). `ListCustomContentParams` adds `id[]`, `space-id[]`, `sort` (`CustomContentSortOrder`). `CreateCustomContentData` accepts `customContentId` and `'raw'` body representation per spec. (B030)
