...
describe('api gap analysis', () => {
  it('resolves all Jira Software operations, including helper-backed issue paths', async () => {
  ...
  it('resolves helper-built Confluence paths and classifies the REST v1 upload', async () => {
    const { stdout, stderr } = await execFileAsync('python3', [ANALYZER], {
      cwd: REPO_ROOT,
    });
  ...
  it('fails when a pinned spec contains an uncovered non-deprecated operation', async () => {
