...
- Completed CLI/skill parity for current and legacy Jira search parameters, including expansions, returned properties, field-key mode, reconciliation IDs, GET fail-fast, legacy validation modes, and offsets.
...
- Corrected Confluence page creation to the current conditional contract: only `spaceId` is unconditional, draft pages may omit `title`, and both flat and representation-keyed nested bodies are supported through the SDK and CLI.
...
- Promoted the route-gap analyzer to a scheduled CI gate: invalid/empty specs or operations, root/path/operation server-scope drift, unresolved or unexpected SDK routes, and uncovered non-deprecated operations now fail the job. Runtime prefixes, assigned resource wiring, lexically visible path states, semantically verified route-preserving helper returns, request properties, and actual live request calls are located in a code-token-only lexical view and resolved from aligned source literals. Comments, literals, regexes, dead branches/calls/wiring, runtime route transformations, unsafe direct or destructuring reassignments, computed properties, and route-overriding spreads cannot produce a false-green report. All eight OpenAPI HTTP operation keys are audited, unresolved Path Item references fail explicitly, and the Confluence REST v1 exception is scoped to the exact attachment-upload verb/path.
