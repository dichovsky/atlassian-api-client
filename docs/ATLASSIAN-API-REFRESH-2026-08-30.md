...
- Promoted the route-gap analyzer to a scheduled CI gate: invalid/empty specs, server-scope drift, unresolved or unexpected SDK routes, and uncovered non-deprecated operations now fail the job. Runtime prefixes, resource wiring, lexically visible path assignments, unambiguous helper returns, request properties, and actual request calls are located in a code-token-only lexical view and resolved from aligned source literals; comments, literals, ambiguous expressions, and route-overriding spreads cannot produce a false-green report.
