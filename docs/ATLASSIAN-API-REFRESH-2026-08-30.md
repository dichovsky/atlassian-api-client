...
- Promoted the route-gap analyzer to a scheduled CI gate: unresolved SDK routes and uncovered non-deprecated operations now fail the job. Runtime prefixes are derived from the actual client initializers and resolution fails closed; a code-token-only lexical view prevents comments or request-like string examples from producing a false-green report.
