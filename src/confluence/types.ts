...
//
// The `/pages/{id}/…` collection family mirrors the blog-post sub-resources:
// ancestors / descendants / direct-children, classification level, custom
// content, likes (count + users), operations, redaction, title update, and
// content-property collections. We re-use the shared content-property and
// redaction types where shapes match; page-specific shapes (children sort
// order, title body) live below.
...
// shape used by folders / databases / blog-posts / comments / attachments:
// the property entity is `ContentProperty`, the listing accepts the shared
// `ListSharedContentPropertiesParams` (key / sort / cursor / limit), the
// create body is `CreateContentPropertyData`, and the update body is
// `UpdateSharedContentPropertyData` (which enforces optimistic concurrency
// via `version.number = current + 1`).
