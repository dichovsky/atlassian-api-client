...
- [x] 🔴 🐛 CLI: B1042 Input-validation cluster (deep-audit 2026-06-10) — PR #259 (2026-06-13)
  - **Impl:** New `asIntArray` helper (rejects non-positive-integer CSV) applied to all 9 array `.map(Number)` sites in `src/cli/commands/jira.ts` incl. the 2 un-named holdouts (`jql autocomplete-data-post --project-ids`, `jql match-issues --issue-ids`); ~14 fields-context scalar `Number()` sites switched to the existing validated helpers. `printJson(undefined)` now emits `null` (valid JSON), not the bare word `undefined`. `resolveAuthType` case-normalised so `Bearer`/`BEARER` resolve to bearer (unknown/empty still default to basic — tested back-compat preserved). `runCli` body wrapped in try/catch → never rejects; any error → `printError` + exit 1, with an `Invalid arguments:` prefix on parseArgs errors (detected via `error.code`, not `.message`). No `router.ts` change needed (the catch around `parseCommand` covers negative-flag `TypeError`).
  - **Rat:** Reject malformed input client-side instead of sending `NaN`/invalid JSON on the wire; make `runCli`'s `Promise<number>` contract truthful. Fail-fast on unknown `--auth-type` was deliberately rejected to preserve a tested decision. Opus-reviewed (APPROVE-WITH-NITS); review caught a dead `.message` branch in the parseArgs-prefix logic, fixed to check `.code`.
- [x] 🔴 🐛 CLI: B1043 `blog-posts list` flags unwired → pagination broken — PR #260 (2026-06-13)
  - **Impl:** `blog-posts list` brought to full `pages list` parity — wires `--title/--status/--body-format/--cursor` + `--sort` (added optional `sort?: BlogPostSortOrder` to `ListBlogPostsParams`; resource auto-forwards via `withSpaceIdParam`, no resource-code change). `comments create` wires `--blog-post-id` and requires exactly one of `--page-id`/`--blog-post-id`; `comments list` wires `--limit/--cursor/--body-format` and `--blog-post-id` de-documented (page-scoped path); `labels list` wires `--cursor`. Skill doc `skill/reference/confluence.md` updated to match (incl. correcting the `blog-posts list --status` filter enum to the spec's `current,deleted,trashed`).
  - **Rat:** Cursor pagination was unusable from the CLI; bring the blog-posts list surface to parity with its pages sibling and make handler/doc/spec consistent. `ListLabelsParams.sort` deliberately untouched (B1021); spec `id` filter excluded (follow-up). Opus-reviewed (APPROVE-WITH-NITS); review caught a wrong `--status` enum in the skill doc + a missing `sort` wire-test, both fixed.
...
- [x] 🟢 🖥️ CLI: B1018 Confluence: expose `labels` space/blogpost listing
  - **Impl:** Branch `feat/cli-confluence-listing` (2026-06-05); added `list-for-space` and `list-for-blog-post` discrete actions to `executeLabels` in `src/cli/commands/confluence.ts`; both dispatch to existing `client.labels.listForSpace` / `client.labels.listForBlogPost` SDK methods (taking `--prefix`, `--sort` via `LABEL_PREFIXES`/`LABEL_SORT_ORDERS`, `--limit`, `--cursor`); `labels` resources line updated in `help.ts`; 2 new action rows + `list-for-space`/`list-for-blog-post` note + examples added to `skill/reference/confluence.md`; matrix row updated; 10 CLI tests (happy-path + flag-forwarding + flag-omission + missing-ID, for each action). No router flag additions needed (all flags pre-existing).
  - **Rat:** `LabelsResource.listForSpace` and `listForBlogPost` were already implemented in the SDK but the `labels` CLI dispatcher only exposed inverse relations (given a label, find content). The new actions expose the forward direction: given a space or blog post ID, list labels on it.
- [x] 🟢 🖥️ CLI: B1019 Confluence: expose `pages` footer/inline comment listing
  - **Impl:** Branch `feat/cli-confluence-listing` (2026-06-05); added `footer-comments` and `inline-comments` actions to `executePages`; dispatches to `client.pages.listFooterComments` / `client.pages.listInlineComments` mirroring the `blog-posts` counterparts exactly (`--body-format`, `--status`, `--sort` + `--resolution-status` for inline, `--cursor`, `--limit`); `pages` resources line updated in `help.ts`; 3 new rows + notes added to `skill/reference/confluence.md` pages section table; matrix row updated; 10 CLI tests (happy-path + full-flag-forwarding + omission + missing-ID for each action). No router flag additions needed.
  - **Rat:** `PagesResource.listFooterComments`/`listInlineComments` existed in the SDK; `blog-posts` already exposed them. Mirroring the same surface on `pages` closes the parity gap.
- [x] 🟢 🖥️ CLI: B1020 Confluence: expose `pages versions` list action
  - **Impl:** Branch `feat/cli-confluence-listing` (2026-06-05); added `versions` action to `executePages` dispatching to `client.versions.listForPage(pageId, { cursor, limit })`; `ListVersionsParams` only accepts `limit`/`cursor` (no `sort`/`body-format`); `pages` resources line updated in `help.ts`; 1 new action row + note added to `skill/reference/confluence.md` pages section; matrix row updated; 3 CLI tests (happy-path + limit/cursor forwarding + missing-ID). No router flag additions needed.
  - **Rat:** `blog-posts` already had `versions` (list); `pages` only exposed a single `version` (get by number). `VersionsResource.listForPage` was already implemented — wiring the CLI action completes the parity.
...
- [x] 🔴 🧩 API: B841 Jira: expose POST /rest/api/3/workflow/history
  - **Impl:** `WorkflowsResource.readWorkflowFromHistory(body)` → `WorkflowHistoryReadResponse` (`{ statuses?, workflows? }`). Body: `{ workflowId (required), version? }`. CLI: `atlas jira workflows read-history --workflow-id <uuid> [--version-number <n>]`. Validates workflowId not empty.
  - **Rat:** Spec-verified: `WorkflowHistoryReadRequest` / `WorkflowHistoryReadResponseDTO` schemas. POST body returns `WorkflowDocumentDTO[]` + `WorkflowDocumentStatusDTO[]`. No path params. Data expires after 60 days; no data before Oct 30 2025.
- [x] 🔴 🧩 API: B842 Jira: expose POST /rest/api/3/workflow/history/list
  - **Impl:** `WorkflowsResource.listWorkflowHistory(body, params?)` → `WorkflowHistoryListResponse` (`{ entries?: WorkflowHistoryItem[] }`). Body: `{ workflowId }`. Query: `expand` (optional, `includeIntermediateWorkflows`). CLI: `atlas jira workflows list-history --workflow-id <uuid> [--expand includeIntermediateWorkflows]`.
  - **Rat:** Spec-verified: `WorkflowHistoryListRequest` / `WorkflowHistoryListResponseDTO` schemas. Single `expand` query param. Each entry: `{ isIntermediate, workflowId, workflowVersion, writtenAt }`.
- [x] 🔴 🧩 API: B843 Jira: expose GET /rest/api/3/workflow/rule/config
  - **Impl:** `WorkflowsResource.getTransitionRuleConfigs(params)` → `WorkflowTransitionRuleConfigPage`. Required param: `types` (array of `postfunction|condition|validator`). Optional: `startAt`, `maxResults`, `keys`, `workflowNames`, `withTags`, `draft` (deprecated), `expand`. Array params serialised as CSV in query string. CLI: `atlas jira workflows get-rule-config --types <csv> [--workflow-names ...] [--with-tags ...] [--expand transition]`.
  - **Rat:** Spec-verified: `PageBeanWorkflowTransitionRules` / `WorkflowTransitionRules` / `AppWorkflowTransitionRule` / `RuleConfiguration`. Connect/Forge apps only (403 otherwise). `draft` param deprecated per changelog (removal Nov 2 2026) — surfaced with `@deprecated` JSDoc. Distinct read (`WorkflowTransitionRulesEntry`) vs update (`WorkflowTransitionRulesUpdateEntry`) shapes typed separately.
- [x] 🔴 🧩 API: B844 Jira: expose PUT /rest/api/3/workflow/rule/config
  - **Impl:** `WorkflowsResource.updateTransitionRuleConfigs(body)` → `WorkflowTransitionRulesUpdateErrors` (`{ updateResults }`). Body: `WorkflowTransitionRulesUpdateBody { workflows: WorkflowTransitionRulesUpdateEntry[] }`. CLI: `atlas jira workflows update-rule-config --workflows '<JSON array>'`.
  - **Rat:** Spec-verified: `WorkflowTransitionRulesUpdate` / `WorkflowTransitionRulesUpdateErrors` / `WorkflowTransitionRulesUpdateErrorDetails`. Response is 200 with partial-error details per workflow; not void. Connect/Forge apps only. `draft` field on `WorkflowId` marked deprecated.
- [x] 🔴 🧩 API: B845 Jira: expose PUT /rest/api/3/workflow/rule/config/delete
  - **Impl:** `WorkflowsResource.deleteTransitionRuleConfigs(body)` → `WorkflowTransitionRulesUpdateErrors`. Body: `WorkflowsWithTransitionRulesDetails { workflows: WorkflowTransitionRulesDeleteEntry[] }` — each entry has `workflowId` + `workflowRuleIds[]`. CLI: `atlas jira workflows delete-rule-config --workflows '<JSON array>'`. Reuses `WorkflowTransitionRulesUpdateErrors` response type (same shape as B844).
  - **Rat:** Spec-verified: `WorkflowsWithTransitionRulesDetails` / `WorkflowTransitionRulesDetails`. Connect apps only (no OAuth scope listed). Response schema `WorkflowTransitionRulesUpdateErrors` identical to B844. Path is PUT (not DELETE) to `/rule/config/delete` — unusual but matches spec.
...
- [x] 🔴 🧩 API: B935 Jira: expose DELETE /rest/api/3/workflow/transitions/{transitionId}/properties
  - **Impl:** `WorkflowsResource.deleteTransitionProperty(transitionId, key, workflowName, workflowMode?)` → void. CLI: `atlas jira workflows delete-transition-property <transitionId> --key <k> --workflow-name <n> [--workflow-mode live|draft]`. Query params: `key` (required), `workflowName` (required), `workflowMode` (optional enum live|draft).
  - **Rat:** Spec-verified: all 4 deprecated endpoints share the same path `/rest/api/3/workflow/transitions/{transitionId}/properties`. `deprecated: true` with removal June 1 2026. DELETE returns 200 (not 204); void return. `key` and `workflow-name` flags already in GLOBAL_OPTIONS; added `workflow-mode` and `include-reserved-keys` flags.
- [x] 🔴 🧩 API: B936 Jira: expose GET /rest/api/3/workflow/transitions/{transitionId}/properties
  - **Impl:** `WorkflowsResource.getTransitionProperties(transitionId, workflowName, params?)` → `WorkflowTransitionProperty`. CLI: `atlas jira workflows get-transition-properties <transitionId> --workflow-name <n> [--key] [--workflow-mode] [--include-reserved-keys]`. Returns spec schema `WorkflowTransitionProperty { key?, value (required), id? }`.
  - **Rat:** Spec-verified: `workflowName` required; `key`, `workflowMode` (default 'live'), `includeReservedKeys` (default false) optional. Response schema `WorkflowTransitionProperty` has only `value` as writable; `key` and `id` are readOnly. `deprecated: true`.
- [x] 🔴 🧩 API: B937 Jira: expose POST /rest/api/3/workflow/transitions/{transitionId}/properties
  - **Impl:** `WorkflowsResource.createTransitionProperty(transitionId, key, workflowName, value, workflowMode?)` → `WorkflowTransitionProperty`. CLI: `atlas jira workflows create-transition-property <transitionId> --key <k> --workflow-name <n> --value <v> [--workflow-mode]`. Body: `{ value }`.
  - **Rat:** Spec-verified: `key` and `workflowName` required query params; `workflowMode` optional (default 'live'). requestBody `WorkflowTransitionProperty` requires only `value` field (key/id are readOnly). `deprecated: true`.
...
- [x] 🟢 📦 Core: B017 Proactive rate-limit (token bucket)
  - files: `src/core/rate-limiter.ts`, `src/core/errors.ts`, `src/core/index.ts`, `src/index.ts`, `test/core/rate-limiter.test.ts`, `README.md`
  - deps: B010 — original backlog listed "deps B010" (circuit breaker); under the middleware-factory design B017 is fully independent of B010 (no shared files, no shared types).
  - **Impl:** `createRateLimiterMiddleware(options: RateLimiterOptions): Middleware` added to `src/core/rate-limiter.ts`. Continuous token-bucket refill; bucket starts full; waits (sleepWithAbort) when empty; throws `RateLimiterExhaustedError` (code `RATE_LIMITER_EXHAUSTED`, `src/core/errors.ts`) when wait would exceed `maxWaitMs`; serialises concurrent requests on a promise chain so no double-consume. `RateLimiterOptions` validated with ValidationError (mirror cache.ts style). Exported from `src/core/index.ts` and `src/index.ts`. README middleware section updated with usage, key behaviours, ordering note, and abort/retry semantics.
  - **Rat:** Opt-in middleware factory; smooths outbound traffic (proactive) vs reactive 429 path; reuses `sleepWithAbort` from retry.ts; no CLI impact; no skill impact.

- [x] 🟡 📦 Core: B015 RS256 Connect JWT
